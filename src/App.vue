	...
	<div id="app" :class="isIndex?'bodybg1':'bodybg2'">
		<div style="width: 100%;" v-show="showHeader">
			<div class="head">
					...
					<img style="width: 50px;height: 50px;display:none" :src="pools.logo" alt="">
					<p style="font-size:16px;font-weight:bold">{{coinName}}</p>
					<!-- <h3 @click="$router.push('/')">首页</h3> -->
					<div @click="goIndex('/')" class="item" :style="tab==2&&{fontWeight:'400'}">
						首页
						<span v-show="tab==1"></span>
					</div>
					<div class="item" :style="tab==1&&{fontWeight:'400'}" @click="goPools">
						矿池
						<span v-show="tab==2"></span>
					</div>
					<!-- <div class="item">
						<p>DAO</p>
						<img style="position: absolute;" src="./assets/nokf.png" alt="">
					</div> -->
					<!-- <h3 @click="$router.push('/pools')">矿池</h3> -->
					<img src="./assets/wallet.png" style="width:20px;height:20px" @click="showWallet=true"/>
		...
		<van-overlay :show="showWallet"  @click="showWallet = false">
			<div class="card">
				<div @click.stop class="wallet">
					<h3 style="color: rgb(50,77,86);text-align: center;font-size:18;font-weight:bold">我的钱包</h3>
					...
					<!-- </div> -->
					<h1 style="color: rgb(50,77,86);text-align: center;font-size:28;font-weight:bold;margin-top:30px;margin-bottom:40px;word-break: break-all;">{{balance}}</h1>
					<!-- <p style="text-align: center;color: #CCCCCC;">Orange余额</p> -->
					<div @click="toTronScan"
					class="dialogBtn">
						在TronScan查看
					</div>
					
					<div @click="showWallet = false" class="dialogBtn1">
						返回页面
					</div>
				</div>
			</div>
		...
		mounted() {
			...
			var timer = setInterval(async () => {
				if (typeof tronWeb !== 'undefined') {
					...
					this.tronScanUrl = 'https://tronscan.org/#/address/' + tronWeb.defaultAddress.base58
